  ...
  if (!seller) {
    return (
      <div className="rounded-2xl border border-black/10 bg-white p-6 shadow-sm">
        <p className="text-sm text-bpGraphite/80">Lojista não localizado.</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex w-full flex-col gap-8">
      <div className="rounded-3xl border border-black/10 bg-white p-8 shadow-sm">
        ...
        <p className="mt-2 text-sm text-bpGraphite/80">Status: {statusLabel(seller.status)}</p>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        <div className="rounded-2xl border border-black/10 bg-white p-6 shadow-sm">
          ...
          <div className="mt-4 grid gap-4 text-sm text-bpGraphite/80 md:grid-cols-2">
            <div>
              <p className="text-xs uppercase tracking-[0.3em] text-bpGraphite/70">
                Responsável
              </p>
              <p className="mt-1 text-sm text-bpBlackSoft">
      ...
      <div className="rounded-2xl border border-black/10 bg-white p-6 shadow-sm">
        <h2 className="font-display text-2xl text-bpBlack">Produtos</h2>
        {products.length === 0 ? (
          <p className="mt-4 text-sm text-bpGraphite/80">
            Nenhum produto cadastrado para este lojista.
          </p>
        ) : (
          <div className="mt-4 grid gap-3 text-sm text-bpGraphite/80">
      ...
      <div className="rounded-2xl border border-black/10 bg-white p-6 shadow-sm">
        <h2 className="font-display text-2xl text-bpBlack">Pedidos</h2>
        {subOrders.length === 0 ? (
          <p className="mt-4 text-sm text-bpGraphite/80">
