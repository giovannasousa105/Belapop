...
export default function ContaFavoritosPage() {
  ...
  const ratingMap = useMemo(() => {
    return products.reduce<Record<string, { avg: number; count: number }>>((acc, product) => {
  ...
  return (
    <div className="space-y-6">
      <PageHeading title="Favoritos" subtitle="Sua seleção salva para voltar, comparar e decidir melhor." />
      {loading ? (
        <p className="text-sm text-bpGraphite/70">Carregando favoritos...</p>
      ) : products.length === 0 ? (
        <div className="rounded-2xl border border-black/10 bg-white p-6 text-sm text-bpGraphite/80">
          Nenhum item salvo ainda. Explore a loja e guarde os produtos que quer rever depois.
        </div>
      ) : (
        <div className="space-y-4">
          <div className="rounded-2xl border border-black/10 bg-white p-4 text-xs text-bpGraphite/70">
            Favoritos salvos: <span className="font-semibold text-bpBlackSoft">{products.length}</span>
            <span className="ml-2">
              Total estimado:{" "}
