...
export async function POST() {
  ...
  if (!email) {
    return NextResponse.json({ error: "E-mail não disponivel para verificação." }, { status: 400 });
  }
  ...
  return NextResponse.json({ ok: true, message: "E-mail de verificação enviado." });
}
