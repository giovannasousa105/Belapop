    ...
    return (
      <SectionFrame>
        <div className="flex flex-col gap-3">
          <p className="text-xs uppercase tracking-[0.3em] text-noir-500">Acesso restrito</p>
          <h1 className="font-display text-2xl text-noir-950">Faça login como admin</h1>
          <p className="text-sm text-noir-600">
            Use uma conta com role <code>admin</code> para visualizar os KPIs operacionais.
          </p>
          <LuxuryButton tone="retail" href="/login?mode=admin">
            Ir para login
          </LuxuryButton>
        </div>
      </SectionFrame>
    ...
    <div className="flex flex-col gap-8">
      <SectionFrame>
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-[0.3em] text-noir-500">Dashboard administrativo</p>
            <h1 className="font-display text-3xl text-noir-950">Operações BelaPop</h1>
            <p className="text-sm text-noir-600">
            ...
            <div className="rounded-2xl border border-black/10 bg-white p-4 shadow-sm">
              ...
              <div className="mt-3 space-y-2">
                {categoryBars.map((item) => (
                  <div
                    key={item.label}
                    className="flex items-center justify-between rounded-xl border border-black/5 bg-noir-50 px-3 py-2"
                  >
                    <span className="text-sm text-noir-800">{item.label}</span>
                    <span className="text-sm font-semibold text-noir-900">{formatPrice(item.value * 100)}</span>
                  </div>
                ))}
                {categoryBars.length === 0 && (
                  <p className="text-sm text-noir-500">Sem categorias com vendas ainda.</p>
                )}
              </div>
            </div>
          </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
        <SectionFrame>
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs uppercase tracking-[0.3em] text-noir-500">Pendências</p>
              <h2 className="font-display text-xl text-noir-950">Ações imediatas</h2>
            </div>
          </div>
        ...
        <SectionFrame>
          <div className="flex items-center justify-between">
            <div>
              ...
              <h2 className="font-display text-xl text-noir-950">Criar anúncio</h2>
            </div>
