...
import { getPublicAsset, getPublicUrl } from "@/lib/publicEnv";
...
export const Footer = () => {
  ...
  const instagramUrl = getPublicUrl(process.env.NEXT_PUBLIC_INSTAGRAM_URL, "");
  const facebookUrl = getPublicUrl(process.env.NEXT_PUBLIC_FACEBOOK_URL, "");
  const tiktokUrl = getPublicUrl(process.env.NEXT_PUBLIC_TIKTOK_URL, "");
  const youtubeUrl = getPublicUrl(process.env.NEXT_PUBLIC_YOUTUBE_URL, "");
  const logoUrl = getPublicAsset(process.env.NEXT_PUBLIC_LOGO_URL, "/logo.svg");
  const logoDarkUrl = getPublicAsset(process.env.NEXT_PUBLIC_LOGO_DARK_URL, "/logo-dark.svg");
  ...
  return (
    ...
    >
      ...
      <div className="w-full bg-bpPink px-6 py-3 text-sm font-semibold uppercase tracking-[0.25em] text-white shadow-sm">
      ...
      <div className="mx-auto mt-8 flex w-full max-w-7xl flex-col gap-10 px-6">
        ...
        <div className="mt-6 rounded-2xl border border-bpPink/25 bg-bpBlack text-white px-6 py-5 shadow-[0_10px_40px_rgba(194,24,91,0.12)]">
          <div className="flex flex-col gap-3 text-sm">
            ...
            <div className="flex items-center gap-3">
              <span className="rounded-full border border-bpPink/40 bg-white/5 px-4 py-2 text-xs font-semibold text-white">
                Site seguro • SSL
              </span>
            </div>
          </div>
